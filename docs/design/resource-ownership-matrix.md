...
The Janitor makes destructive API calls, so each selection rule is part of its
public behavior. This document records the Python controller's ownership rules
as the maximum candidate boundary for the replacement release.

The Go controller may add checks that fail closed, but it must not broaden the
Python selectors. A wider ownership model needs separate compatibility and
rollout work after the replacement is stable. The [roadmap](../../ROADMAP.md)
tracks implementation status, and the
[CAPO integration boundary](capo-integration-boundary.md) assigns lifecycle
ownership outside this matrix.
...
All discovery and deletion happens through the credential selected from the
`OpenStackCluster` identity Secret.
...
Project and region scoping reduce the candidate inventory, but they do not
replace the resource-specific selectors below.

## Ownership rules

| Resource | Replacement policy | Python selector | Go selector | Delete request |
| --- | --- | --- | --- | --- |
...
| Octavia service load balancer | Follow the [open load balancer decision](cleanup-behaviour-matrix.md#open-replacement-decisions). CAPO API server load balancer status is not ownership proof | `name` starts with `kube_service_<cluster-name>_` | Keep the exact prefix as a required condition and add the agreed no-conflict checks for shared load balancers and reserved tags | Cascade delete by load balancer ID only after every ownership condition passes |
...
The candidate and verification selectors must be the same. A controller must
not request deletion with one rule and later declare success with a weaker rule.
...
The examples below use `demo` as the resolved cluster name. Each positive
fixture passes its resource selector; policy checks may still block deletion.
Each negative fixture remains untouched.
...
The Python selector allows text between the fixed start and end. The Go selector
preserves that behavior and does not replace it with a general substring search.
...
The positive fixture passes only the legacy name selector. The
[load balancer release decision](cleanup-behaviour-matrix.md#open-replacement-decisions)
must add the checks that make it eligible for cascade deletion. CAPO API server
load balancer status is not proof of ownership, and a conflicting shared load
balancer tag is a negative fixture. A CAPO API server load balancer remains
outside this rule.
...
As with floating IPs, the Python selector checks a fixed start and end. The Go
controller keeps those checks exactly.
...
Missing metadata is also negative. Snapshot selection has no independent keep
property in the Python implementation. A keep property on a related volume does
not remove a matching snapshot from the candidate set.
...
The replacement keeps the case-sensitive comparison. Values such as `True`,
`1`, or `yes` do not have the same meaning as `true`.

### Application credential

The candidate is not found by name or by listing all user credentials. The Go
controller uses the exact `application_credential_id` from the selected
`clouds.yaml` entry.

The Python code reads the hard-coded `openstack` entry at this point, even if a
different cloud was selected earlier. Using the selected entry is a correctness
fix, not a wider ownership rule.
...
- all other required resources have been verified absent and that fact has been
  persisted in the Janitor-owned checkpoint

Any missing condition means that the credential and Secret are not deleted in
that reconcile.

## Discovery and deletion rules

Every resource service must list all pages before cleanup can be declared
complete. A pagination or extraction error makes inventory incomplete and must
retain the finalizer.

Implement selection as pure functions over small domain types. Deletion code
receives already-authorized IDs. This keeps API transport details separate from
ownership decisions and makes negative fixture tests easy to review.

Delete requests are idempotent. A resource selected in an earlier reconcile
that is now absent can be treated as complete. A `NotFound` response must not
authorize a resource that never passed the selector.
...
The composed OpenStack end-to-end test creates at least one owned resource and
one similar non-owned resource. It passes only when the owned resource is
removed and the non-owned resource remains.

## Changing an ownership rule

The Python selectors are frozen through the replacement release. A later change
needs:
...
This process applies even when a broader selector appears to fix an obvious
leftover. Shared load balancer and reserved-tag checks that narrow eligibility
are required before the replacement release; broadening the candidate boundary
remains post-replacement work.
