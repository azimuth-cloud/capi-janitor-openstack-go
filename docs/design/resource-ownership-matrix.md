...
All discovery and deletion happens through the direct application-credential Secret resolved from the `OpenStackCluster` identity reference.
The [compatibility policy](python-compatibility-policy.md) defines Secret resolution, cloud selection, and authentication validation.
After that resolution:

- the OpenStack project is the project carried by the authenticated token for the selected credential
- the interface and optional region come from the selected `clouds.yaml` entry
...
The effective cluster name is the primary ownership key for every legacy selector below.
It must exactly match the name used by OCCM and Cinder CSI when they created the resources.
Workload cluster names must be unique within one OpenStack project because the OpenStack resources do not carry an immutable workload-cluster UID.
A mismatch can leave owned resources behind, while a duplicate name can make resources from two clusters indistinguishable.
The Janitor must not use fuzzy matching to compensate for either condition.
...
| Kubernetes credential Secret | Same gate as application credential cleanup | Exact referenced Secret in the `OpenStackCluster` namespace | Only the exact direct application-credential Secret is owned | Delete the recorded Secret with a UID precondition after the qualifying checkpoint outcome |
...
- any identity object, credential, or Secret other than the exact supported direct application-credential Secret
- SSH keypairs
...
These reserved tags are OCCM's official ownership evidence for shared load balancers.
Pool members, member names, and member ports are not ownership evidence.
...
Multiple well-formed target-cluster tags represent Services sharing the LB within the same workload cluster and do not veto deletion.
A well-formed tag for any other cluster preserves the whole LB and its VIP floating IP.
...
The [Octavia error policy](python-compatibility-policy.md#octavia-capability-and-error-handling) requires a usable endpoint and complete inventory before this preflight can succeed.
