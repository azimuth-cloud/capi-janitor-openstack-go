...
func (s *Service) ListLoadBalancers(ctx context.Context) ([]cleanup.LoadBalancer, error) {
	var result []cleanup.LoadBalancer
	...
	err := pager.EachPage(ctx, func(_ context.Context, page pagination.Page) (bool, error) {
		pageLoadBalancers, err := loadbalancers.ExtractLoadBalancers(pageutil.UnwrapCollectionPage(page))
		if err != nil {
			return false, fmt.Errorf("extracting load balancer page: %w", err)
		}
		for _, loadBalancer := range pageLoadBalancers {
			...
			result = append(result, cleanup.LoadBalancer{
				ID:   loadBalancer.ID,
